...
// - As int:  ~11 bytes of source code for 4 bytes of input data. 4 bytes stored in binary. Endianness dependent, need
...
// Versione modificata:
//  * aggiunta la directory di output come parametro;
//  * salva l'output in un file con estensione '.cpp' invece di stamparlo nel stdout;
//  * se l'opzione '-nostatic' è specificata genera un file con estensione '.h' in cui dichiara le variabili come
//  extern;
//  * il file o i file di output hanno lo stesso nome specificato per il simbolo;
//  * il file o i file di output sono in formato UTF-8 con BOM.
...
int main(int argc, char **argv)
{
    ...
    {
        printf(
            "Syntax: %s [-u8|-u32|-base85] [-nocompress] [-nostatic] <inputfile> <namespace> <symbolname> "
            "<output_name>\n",
            argv[0]);
        printf("Source encoding types:\n");
        printf(" -u8     = ~12 bytes of source per 4 bytes of data. 4 bytes in binary.\n");
        printf(
            " -u32    = ~11 bytes of source per 4 bytes of data. 4 bytes in binary. Need endianness swapping on "
            "big-endian.\n");
    ...
    bool ret = binary_to_compressed_c(
        argv[argn], argv[argn + 1], argv[argn + 2], argv[argn + 3], source_encoding, use_compression, use_static);
    ...
    const char *namespace_)
{
    ...
    const char *encoding_str = source_encoding == SourceEncoding_Base85 ? "base85"
                             : source_encoding == SourceEncoding_U8     ? "u8"
                                                                        : "u32";
    ...
    fprintf(
        out,
        "// Exported using BinaryToCompressedC -%s \"%s\" %s \"%s\"\n\n",
        encoding_str,
        filename,
        symbol,
        output_name);
    ...
    bool use_static)
{
    ...
    snprintf(outputfilename, 512, "%s.cpp", output_name);

    // Output as Base85 encoded
    FILE *out = fopen(outputfilename, "wt");
    print_header(out, source_encoding, filename, data_sz, symbol, output_name, namespace_);
    const char *static_extern_str = use_static ? "static " : "";
    const char *compressed_str    = use_compression ? "Compressed" : "";

    const char *nome_font = strrchr(filename, '/');
    if (nome_font == nullptr)
    {
        nome_font = strrchr(filename, '\\');
        if (nome_font == nullptr) nome_font = symbol;
        else nome_font++;
    }
    else nome_font++;
    ...
    fprintf(
        out,
        "        // Workaround to avoid GCC warning 'initialized and declared extern' because this warning can't be "
        "disabled \xEF\xBB\xBF\xF0\x9F\x99\x81" // Codice binario UTF-8 della faccina triste.
        ".\n");
    fprintf(out, "        extern const char *Name;\n");
    fprintf(out, "        extern const unsigned char %sData[%d];\n", compressed_str, (int)compressed_sz);
    fputs("\n", out);
    fprintf(out, "        %sconst char *Name = \"%s\";\n", static_extern_str, nome_font);

    if (source_encoding == SourceEncoding_Base85)
    {
        fprintf(
            out,
            "        %sconst char %sDataBase85[%d+1] =\n    \"",
            static_extern_str,
            compressed_str,
    ...
    else if (source_encoding == SourceEncoding_U8)
    {
        ...
        for (int i = 0; i < compressed_sz; i++)
        {
            ...
            if (column == 0) column += fprintf(out, "\n            ");
            column += fprintf(out, "0x%02X, ", d);
            if (column + 5 >= 120) column = 0;
        }
        fprintf(out, "\n        };\n");
    }
    else if (source_encoding == SourceEncoding_U32)
    {
        ...
        for (int i = 0; i < compressed_sz; i += 4)
        {
            ...
            if ((column++ % 14) == 0) fprintf(out, "\n            0x%08x, ", d);
            else fprintf(out, "0x%08X, ", d);
        }
        fprintf(out, "\n        };\n\n");
    }
    ...
    if (!use_static)
    {
        snprintf(outputfilename, 512, "%s.h", output_name);
